...
binaries: linux_amd64 linux_i386 pi_arm64 pi_arm windows_amd64 windows_i386 mac_amd64
...
windows_amd64:
	GO111MODULE=on CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -v -a -o binaries/$@/${process_name}.exe
	cp README.md binaries/$@/
	# tar files inside the specified release directory
	tar -czvf binaries/${process_name}.$@.tar.gz -C binaries/$@/ ${process_name}.exe README.md

windows_i386:
	GO111MODULE=on CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -v -a -o binaries/$@/${process_name}.exe
	cp README.md binaries/$@/
	# tar files inside the specified release directory
	tar -czvf binaries/${process_name}.$@.tar.gz -C binaries/$@/ ${process_name}.exe README.md
